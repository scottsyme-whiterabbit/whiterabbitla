...
import { Mail, RefreshCw, Send, Loader2, BadgeDollarSign, BellOff } from "lucide-react";
...
const ClientContextPanel = ({ deal, open, onOpenChange, adminPassword, onEditDeal }: Props) => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const [msgs] = await Promise.all([loadThread(deal.id), loadInvoices(deal.id)]);
  ...
  }, [open, deal, loadThread, loadInvoices, syncGmail]);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-3xl bg-background border-border max-h-[92vh] overflow-y-auto">
        <DialogHeader>
          ...
          {(deal.event_type || deal.event_date || deal.location) && (
            <p className="text-xs text-muted-foreground">
              {[
                deal.event_type,
                deal.event_date
                  ? new Date(`${deal.event_date}T12:00:00`).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })
                  : null,
                deal.location,
              ].filter(Boolean).join(" · ")}
            </p>
        ...
        <button
          ...
          className="w-full mt-2 border border-border py-2 font-sans text-[10px] tracking-[0.15em] uppercase text-muted-foreground hover:text-foreground transition-colors"
        >
          Edit deal details
        </button>
      </DialogContent>
