...
const LegacyCityPage = ({ area, citySlug }: Props) => {
  ...
  useWebPageSchema({
    ...
    description: `Hire a world-class magician for luxury events in ${area.city}. Close-up magic, parlor shows, and bespoke entertainment.`,
    path: `/areas/${citySlug}`,
    type: "WebPage",
  });

  const matchingPages = seoPages.filter((p) => p.location === area.city);
        ...
        <div className="absolute inset-0 flex items-end">
          <div className="max-w-4xl mx-auto px-6 pb-12 w-full">
            <AnimatedSection>
              ...
              <h1 className="font-serif text-5xl md:text-6xl text-cream mb-3">{area.city}</h1>
              <p className="font-sans text-base text-cream/70 max-w-lg">{area.tagline}</p>
            </AnimatedSection>
          </div>
        </div>
      </section>
      ...
      {matchingPages.length > 0 && (
        <section className="py-16 border-t border-border">
          <div className="max-w-5xl mx-auto px-6">
            <AnimatedSection>
              <p className="font-sans text-xs tracking-[0.3em] uppercase text-accent mb-2">Services in {area.city}</p>
      ...
      <AnimatedSection>
        <section className="bg-forest-dark py-24 text-center">
          <div className="max-w-2xl mx-auto px-6">
            <h2 className="font-serif text-4xl text-cream mb-6">Book White Rabbit in {area.city}</h2>
